...
exports.home = function (request, response) {
    ...
    connection.query(`SELECT COUNT(*) as totalCnt FROM topic LEFT JOIN author ON topic.author_id = author.id ORDER BY created ASC;`, function (error6, count) {
        if (error6) {
            throw error6;
        }
        //paging
        ...
        var getPageRow = (page * 10) - 10;

        connection.query(`SELECT * FROM topic ORDER BY created ASC LIMIT ${getPageRow}, ${countPage}`, function (error, topics) {
            if (error) {
                console.log(error);
            }
            connection.query(`SELECT * FROM filter`, function (error3, filters) {
                ...
                connection.query(`SELECT * FROM searchfilter`, function (error4, searchfilters) {
                    ...
                    var pagingBox = template.pageHTML(pageArray, queryData.page, startPage, endPage);
                    var list = template.List(topics);
                    var html = template.HTML(title, list, description, `<a href="/create">Create</a>`, filterBox, searchBox, pagingBox);
