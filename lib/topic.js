...
exports.sortingHome = function (request, response) {
    ...
    var orderWord = '';
    if (queryData.id === 1) {
        orderWord = `ORDER BY created ASC`;
    } else if (queryData.id === 2) {
        orderWord = `ORDER BY created DESC`;
    } else if (queryData.id === 3) {
        orderWord = `ORDER BY name ASC `;
    }
    console.log(queryData.id);
    console.log(orderWord);
    var query = connection.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id = author.id ?`, [orderWord], function (error, topics) {
        if (error) {
            console.log(error);
        }
        connection.query(`SELECT * FROM filter`, function (error2, filters) {
